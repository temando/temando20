...
class Ewave_Temando_Model_Api_V2_Client extends Mage_Core_Model_Abstract {
	...
	if (!$username) {
	    $username = Mage::helper('temando/v2')->getConfigData('general/username');
	}
	if (!$password) {
	    $password = Mage::helper('temando/v2')->getConfigData('general/password');
	}
	...
	$this->_client = new DebugSoapClient($url, array('soap_version' => SOAP_1_2, 'trace' => TRUE));
    ...
    /**
     * DEPRECATED IN FAVOUR OF getQuotes API CALL
     * 
     ...
     * @return array
     */
    public function getQuotesByRequest($request, $warehouse_id = null) {
	
	return $this->getQuotes($request, $warehouse_id);
	...
	//Mage::log($request, null, 'temando-v2-request.log', true);
    ...
    public function getQuotes($request, $warehouse_id = null) {
	if (!$this->_client) {
	    return false;
	}
	...
	foreach ($response->quotes->quote as $quote_details) {
	    $quotes[] = Mage::getModel('temando/quote')
		    ->loadResponse($quote_details, $packages, $warehouse_id);
	}
	...
	return $this->_client->getPickupLocations($request);
    }
