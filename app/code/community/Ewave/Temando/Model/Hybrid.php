...
class Ewave_Temando_Model_Hybrid extends Mage_Core_Model_Abstract
{
	...
	foreach($collection->getItems() as $rule) {
		...
		continue;
	     ...
	     //don't load static rules if pickup location and pickup does not allow static rates
	    if($rule->isStatic() && $this->getIsPickup() && !$this->_helper->getConfigData('pickup/allow_static')) {
		continue;
	    }
    ...
    public function getShippingMethods(&$error, $options, $quotes = null, $origin_id)
    {
