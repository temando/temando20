...
class Ewave_Temando_Model_Observer
{
	...
	preg_match('/^temando_multi:/', $shippingMethod) ? $isSplit = true : $isSplit = false;
	...
	$selected_delivery_options = Mage::getSingleton('checkout/session')->getData('selected_delivery_options');
	...
	if(!$selected_quote->getId() && !$isSplit) {
	...
	if($isSplit) {
	    //SPLIT Shipment - virtual shipping method (combined rate)
	    $selected_quote = null;
	    $combined_rate = Mage::getSingleton('checkout/session')->getData('combined_rate');
	    preg_match('/insurance_(Y|N)_carbonoffset_(Y|N)/', $shippingMethod, $selected_options);
	    if($combined_rate instanceof Mage_Shipping_Model_Rate_Result_Abstract)
	    {
		$temando_shipment->setIsSplit(1)
				 ->setCustomerSelectedQuoteDescription('Multi Origin Shipment')
				 ->setCombinedRate(serialize($combined_rate))
				 ->setCustomerSelectedOptions(!empty($selected_options) ? $selected_options[0] : 'insurance_N_carbonoffset_N');
	    }		
	} else {
	...
	$warehouse_id = null; $unavailable = array();
	if(Mage::helper('temando')->isVersion2()) {
	    $origins = Mage::helper('temando/v2')->getDynamicOrigins(
		...
		$ids = array_keys($origins);
		$warehouse_id = (int)$ids[0]; //one saved is enough
	    }
	}
	
	$isPickup = Mage::getSingleton('checkout/session')->getData('temando_is_pickup');
	$customer_comment = Mage::getSingleton('checkout/session')->getData('customer_comment');
	$shipping_instructions = Mage::getSingleton('checkout/session')->getData('shipping_instructions');
	
	if(Mage::app()->getRequest()->getModuleName() == 'onestepcheckout') {
	    $customer_comment = Mage::app()->getRequest()->getPost('comment', null);
	    $shipping_instructions = Mage::app()->getRequest()->getPost('instructions', null);
	}
	    ...
	    ->setPickupLocation($isPickup ? Mage::getSingleton('checkout/session')->getData('temando_pickup_location') : null)
	    ->setAvailablePickupLocations($isPickup ? serialize(Mage::getSingleton('checkout/session')->getData('temando_pickup_locations')) : null)
	    ->setCustomerSelectedDeliveryOptions(implode(',', is_array($selected_delivery_options) ? array_keys($selected_delivery_options): array()))
	    ->setCustomerComment($customer_comment)
	    ->setShippingInstructions($shipping_instructions)
            ->save();
	
	if($isSplit) {
	    $temando_shipment->clearQuotes();
	}
