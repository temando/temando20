...
class Ewave_Temando_Helper_V2 extends Ewave_Temando_Helper_Data {
    ...
    {
	return Mage::getSingleton('temando/warehouse')->getCollection()
		->getOriginByPostcode($postcode, $storeId);
    }
    
    /**
     * Returns array of origins able to serve current request
     * 
     ...
     * @return array 
     */
    public function getDynamicOrigins($postcode, $storeId = null, $items, $err = array())
    {
	return Mage::getSingleton('temando/warehouse')->getCollection()
		->getDynamicOrigins(
			$postcode, 
			$storeId, 
			$this->getProductIdsFromRequestItems($items), 
			$err
		);
    }
    
    /**
     * Returns list of product IDs on order
     * (for configurable products, selected simple product id is returned)
     * 
     * @param mixed $items
     * @return array 
     */
    public function getProductIdsFromRequestItems($items)
    {
	$productIds = array();
