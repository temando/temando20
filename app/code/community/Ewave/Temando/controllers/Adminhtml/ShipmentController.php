...
class Ewave_Temando_Adminhtml_ShipmentController extends Mage_Adminhtml_Controller_Action {
	...
	if ($data = $this->getRequest()->getPost()) {
	    ...
	    if (isset($data['qty_to_ship'])) {
		Mage::getSingleton('adminhtml/session')->setData('qty_to_ship_' .$this->getRequest()->getParam('id'), $data['qty_to_ship']);
	    } 
	...
	$shipment = Mage::getModel('temando/shipment')->load($shipment_id);
	/* @var $shipment Ewave_Temando_Model_Shipment */

	$quote_id = $this->getRequest()->getParam('quote');
	$quote = Mage::getModel('temando/quote')->load($quote_id);
	/* @var $quote Ewave_Temando_Model_Quote */
	
	$shipQuantities = Mage::getSingleton('adminhtml/session')->getData("qty_to_ship_$shipment_id");
	if(!is_array($shipQuantities)) $shipQuantities = array();

	if(!$shipment->getOrder()->canShip()) {
	    $this->_getSession()->addError('Cannot create shipment for order.');
	    $this->_redirect('*/*/edit', array('id' => $shipment_id));
	    return;
	}
	
	$shipmentOrigin = Mage::getModel('temando/warehouse')->load($shipment->getWarehouseId());
	/*@var $shipmentOrigin Ewave_Temando_Model_Warehouse */
	
	$totalQty = $totalShipToQty = 0;
	foreach ($shipment->getOrder()->getAllItems() as $item) {
	    if ($item->getQtyToShip() && !$item->getIsVirtual()) {		
		if(!$shipmentOrigin->hasProduct($item->getProductId())) {
		    continue;
		}
		$qtyToShip = array_key_exists($item->getProductId(), $shipQuantities) ?  $shipQuantities[$item->getProductId()] : null;
		if(is_null($qtyToShip)) {
		    $qtyToShip = $item->getQtyToShip();
		}
		$qty = $qtyToShip > $item->getQtyToShip() ? $item->getQtyToShip() : $qtyToShip;
		$totalShipToQty += $qty;
	...
	if($totalShipToQty <= 0) {
	    $this->_getSession()->addError('Cannot create an empty shipment. Please check \'Qty to Ship\' field on individual items.');
	    $this->_redirect('*/*/edit', array('id' => $shipment_id));
	    return;
	}
	...
	if (!$error && $booking_result) {
	    ...
	    if (!isset($booking_result->bookingNumber))		    { $booking_result->bookingNumber = null; }
	    if (!isset($booking_result->consignmentNumber))	    { $booking_result->consignmentNumber = null; }
	    if (!isset($booking_result->consignmentDocument))	    { $booking_result->consignmentDocument = null; }
	    if (!isset($booking_result->consignmentDocumentType))   { $booking_result->consignmentDocumentType = null; }
	    if (!isset($booking_result->requestId))		    { $booking_result->requestId = null; }
	    if (!isset($booking_result->labelDocument))		    { $booking_result->labelDocument = null; }
	    if (!isset($booking_result->labelDocumentType))	    { $booking_result->labelDocumentType = ''; }
	    ...
	    $magento_shipment = Mage::getModel('sales/convert_order')->toShipment($shipment->getOrder());
		...
		if ($item->getQtyToShip() && !$item->getIsVirtual()) {		    
		    ...
		    $magento_shipment_item = Mage::getModel('sales/convert_order')->itemToShipmentItem($item);
		    $qty = $qtyToShip > $item->getQtyToShip() ? $item->getQtyToShip() : $qtyToShip;
	    ...
	    $magento_shipment->setTotalQty($totalShipToQty);
	    ...
	    $magento_shipment
		    ->addTrack($track)
		    ->setIsTemando(1)
		    ->setTemandoShipmentOrigin(Mage::getModel('temando/warehouse')->load($shipment->getWarehouseId())->getName())
		    ->setTemandoShipmentReadyDate(isset($booking_result->anytime)?(string) $booking_result->anytime->readyDate:null)
		    ->setTemandoShipmentReadyTime(isset($booking_result->anytime)?(string) $booking_result->anytime->readyTime:null)
		    ->setTemandoLabelDocument($booking_result->labelDocument)
		    ->setTemandoLabelDocumentType($booking_result->labelDocumentType)
		    ->setTemandoConsignmentDocument($booking_result->consignmentDocument)
		    ->setTemandoConsignmentDocumentType($booking_result->consignmentDocumentType)
		    ->register();
		...
		//change Temando shipment status
		$magento_shipment->getOrder()->canShip() ? 
			$shipment->setStatus(Ewave_Temando_Model_System_Config_Source_Shipment_Status::PART_BOOKED) :
			$shipment->setStatus(Ewave_Temando_Model_System_Config_Source_Shipment_Status::BOOKED);
		$shipment->save();	
		$magento_shipment->sendEmail();
		Mage::getSingleton('adminhtml/session')->unsetData("qty_to_ship_$shipment_id");
	...
	$delivery_options = explode(',',$shipment->getCustomerSelectedDeliveryOptions());
	...
	$request
		...
		->setDeliveryOptions(array_flip($delivery_options));
	...
	$comment = $shipment->getCustomerComment();
	if($comment) {
	    $request_array['comments'] = $comment;
	}
	...
	if ($booking_result) {
		...
		if ($item->getQtyToShip() && !$item->getIsVirtual()) {
		    ...
		    $qtyToShip = (int)$this->getRequest()->getPost("qty_to_ship_{$productId}", null);
		    if(!$qtyToShip) $qtyToShip = $item->getQtyToShip();
...
//    }
	...
	if($shipment) {
	    ...
	    switch($labelType) {
		case Ewave_Temando_Model_System_Config_Source_Labeltype::STANDARD:
		    if($shipment->getTemandoConsignmentDocument()){
			$document = base64_decode($shipment->getTemandoConsignmentDocument());
			$documentType = $shipment->getTemandoConsignmentDocumentType();
		    }
		    break;
		case Ewave_Temando_Model_System_Config_Source_Labeltype::THERMAL:
		    if($shipment->getTemandoLabelDocument()){
			$document = base64_decode($shipment->getTemandoLabelDocument());
			$documentType = $shipment->getTemandoLabelDocumentType();
		    }
		    break;
	    }
	    ...
	    if ($document_length) {
		if(!$documentType) {$documentType = 'application/pdf';}
		$this->getResponse()
			->setHttpResponseCode(200)
			->setHeader('Pragma', 'public', true)
			->setHeader('Cache-Control', 'must-revalidate, post-check=0, pre-check=0', true)
			->setHeader('Content-type', $documentType, true)
			->setHeader('Content-Length', $document_length)
			->setHeader('Content-Disposition', 'attachment; filename="shipment-' . $shipment->getIncrementId() . '.pdf"')
			->setHeader('Last-Modified', date('r'));
		$this->getResponse()->clearBody();
		$this->getResponse()->sendHeaders();
		print $document;
	...
	if(!$shipment) {
	    $this->_getSession()->addError('Shipment no longer exists.');
	    $this->_redirect('*/*/edit', array('id' => $this->getRequest()->getParam('id')));
	    return;
	}
	
	if(!$document_length) {
	    $this->_getSession()->addError('Consignment document not found for shipment '. $shipment->getIncrementId() .'.');
	    $this->_redirect('*/*/edit', array('id' => $this->getRequest()->getParam('id')));
	    return;
	}
    }
