...
class Ewave_Temando_Block_Adminhtml_Shipment_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    public function __construct()
    {
        ...
        if ($this->getShipment()->getStatus() == Ewave_Temando_Model_System_Config_Source_Shipment_Status::PENDING ||
...
//        if ($this->getShipment()->getStatus() == Ewave_Temando_Model_System_Config_Source_Shipment_Status::BOOKED) {
//            $class = 'go';
...
//                'label' => $title,
//                'id' => 'consignment',
//                'onclick' => "window.location = '" . $this->getUrl('*/*/consignment', array('id' => $this->getRequest()->getParam('id'))) . "'",
//                'value' => '',
//                'class' => $class,
//            ));
//        }
     ...
     *
     * @return Ewave_Temando_Model_Shipment
     */
    ...
    protected function _getProductsJson($variable_name)
    {
        ...
        foreach ($this->getShipment()->getOrder()->getAllItems() as $item) {
            ...
            $helper->applyTemandoParamsToProductByItem($item, $product);
